        ...
        <div class="flex flex-col lg:flex-row gap-12 lg:gap-20 items-center">
            ...
            <div class="w-full lg:w-1/2 relative group">
                ...
                <div class="relative bg-gray-50 rounded-3xl p-8 md:p-12 overflow-hidden shadow-2xl shadow-[#0A4088]/10 border border-gray-100">
                    @if($product->image)
                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->nama_produk }}" class="w-full h-auto max-h-125 object-contain transform group-hover:scale-105 transition-transform duration-700">
                    @else
                        <img src="{{ asset('images/Rectangle24.png') }}" alt="{{ $product->nama_produk }}" class="w-full h-auto max-h-125 object-contain transform group-hover:scale-105 transition-transform duration-700">
                    @endif
    ...
    <div class="bg-gray-50 py-20 md:py-32 border-t border-gray-100">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            ...
            @if($product->link_portofolio)
                <a href="{{ $product->link_portofolio }}" target="_blank" class="inline-flex items-center justify-center gap-3 bg-[#0A4088] hover:bg-[#08306b] text-white px-10 py-5 rounded-2xl font-bold shadow-xl shadow-[#0A4088]/20 transition-all transform hover:-translate-y-1 active:scale-95">
