...
<div class="max-w-4xl mx-auto pb-20 px-4 sm:px-6 lg:px-8 pt-4 sm:pt-8">
    ...
    @if ($errors->any() || session('error'))
        <div class="mb-8 p-6 bg-red-50 border border-red-100 rounded-2xl">
            <div class="flex items-start gap-4">
                <i data-lucide="alert-circle" class="w-6 h-6 text-red-500 mt-0.5 shrink-0"></i>
                <div>
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white p-6 sm:p-8 md:p-10 rounded-3xl border border-gray-100 shadow-sm space-y-6">
            ...
            <div class="space-y-2">
                <label for="deskripsi" class="block text-xs font-bold text-gray-400 uppercase tracking-widest">Description</label>
                <textarea id="deskripsi" name="deskripsi" rows="4"
                class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A4088]/20 focus:border-[#0A4088] transition">{{ old('deskripsi') }}</textarea>
            </div>
        </div>
