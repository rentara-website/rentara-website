    ...
    <form action="{{ route('admin.products.update', $product->slug) }}" method="POST" enctype="multipart/form-data" class="space-y-8">
        ...
        <div class="bg-white p-8 md:p-10 rounded-3xl border border-gray-100 shadow-sm space-y-6">
            ...
            <div class="space-y-2">
                ...
                class="w-full bg-gray-50 border border-gray-100 rounded-2xl px-5 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#0A4088]/20 focus:border-[#0A4088] transition">{!! $product->deskripsi!!}</textarea>
            </div>
        </div>
