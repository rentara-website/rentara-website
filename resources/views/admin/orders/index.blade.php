                ...
                <tbody class="divide-y divide-gray-50 text-xs font-bold">
                    @forelse($orders as $order)
                        <tr class="hover:bg-gray-50/50 transition">
                            ...
                            <td class="px-8 py-5 text-gray-900">{{ $order->booking_date->format('d M Y, H:i') }}</td>
                            ...
                            <td class="px-8 py-5 text-right">
                                <form action="{{ route('admin.orders.destroy', $order->id) }}" method="POST" onsubmit="return confirm('Are you sure you want to remove this order record?')">
                                    @csrf
                                    @method('DELETE')
